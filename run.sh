        ...
        do
            ...
            do
                ...
                if [ -f model/$dataset-$Bi-$seed-cross-scl_filter_${filter_t}_t${filter_threshold#0.}/Test.json ]; then
                    ...
                    CUDA_VISIBLE_DEVICES=0 python ContrastLearning.py --dataset $dataset --BiLSTM $Bi --seed $seed --filter_threshold $filter_threshold --weight $weight --full-training
                fi
                done
            done
        done
    ...
    do
        ...
        do
            ...
            if [ -f model/$dataset-$Bi-$seed-cross-scl_filter_${filter_t}_t${filter_threshold#0.}/Test.json ]; then
                ...
                echo "model/$dataset-$Bi-$seed-cross-scl_filter_${filter_t}_t${filter_threshold#0.} does not exist"
                CUDA_VISIBLE_DEVICES=0 python ContrastLearning.py --dataset $dataset --BiLSTM $Bi --seed $seed --filter_threshold $filter_threshold --weight=0.7
            fi
            done
        done
    done
