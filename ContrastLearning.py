...
def run(args):
    ...
    if time_aware==0:
        dataset = Data_Generator(file,MAX,seqfile=f"datasets/seq_bin{int(args.bin)}_{args.filter}_t{int(args.filter_threshold*100):02d}",catfile=f"datasets/cat_bin{int(args.bin)}_{args.filter}_t{int(args.filter_threshold*100):02d}")
